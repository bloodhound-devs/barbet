    ...
    for seed_num, seed in enumerate(range(random_seed, random_seed+num_seeds)):
        ...
        for model_num, (model_name, model_runner, params) in enumerate(model_runner_and_params):
            ...
            if prediction_callback is not None:
                prediction_callback(unrelaxed_protein, sequences_lengths,result, input_features, (tag, False))
...
def build_monomer_feature(sequence: str, unpaired_msa: str, template_features: Dict[str, Any]):
    ...
    **kwargs
    ...
    for job_number, (raw_jobname, query_sequence, a3m_lines) in enumerate(queries):
        ...
        try:
            ...
            if pickled_msa_and_templates.is_file():
                ...
                if a3m_lines is None:
                    ...
                    = get_msa_and_templates(jobname, query_sequence, a3m_lines, result_dir, msa_mode, use_templates, custom_template_path, pair_mode, pairing_strategy, host_url, user_agent)
        ...
        try:
            ...
            models_embeddings = predict_structure(
                prefix=jobname,
...
def run_batch_colabfold(cf_kwargs: dict = None):
    cf_kwargs = {} if cf_kwargs is None else cf_kwargs
    queries = cf_kwargs.get('queries', [])
    print(f"Running batch colabfold on {len(queries)} sequences: {queries}")
    return run(**cf_kwargs)
